            ...
            {allProjects.map((proj, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 10, padding: '14px 18px', border: '1px solid #e5e7eb', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontFamily: "'Fira Code', monospace", fontWeight: 700, fontSize: 14, color: '#1a1a2e', marginBottom: 1 }}>{proj.title}</p>
                  <p style={{ fontSize: 13, color: '#9ca3af', fontFamily: "'Fira Code', monospace", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{proj.description}</p>
                </div>
                <button
                  onClick={() => router.push(`/profile/projects?edit=${i}`)}
                  style={{ background: 'none', border: '1.5px solid #e5e7eb', borderRadius: 7, padding: '5px 12px', fontSize: 12, color: '#6b7280', cursor: 'pointer', fontFamily: "'Fira Code', monospace", fontWeight: 600, flexShrink: 0 }}
                >✏️ Edit</button>
                <button
                  onClick={() => handleDelete(i)}
                  disabled={loading}
                  style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 7, padding: '5px 12px', fontSize: 12, color: '#ef4444', cursor: 'pointer', fontFamily: "'Fira Code', monospace", fontWeight: 600, flexShrink: 0 }}
                >🗑 Delete</button>
              </div>
            ))}
          </div>
              ...
              <div key={i} style={{ border: '1.5px solid #e5e7eb', borderRadius: 12, padding: '20px 22px', marginBottom: 16 }}>
                {!isEditMode && entries.length > 1 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <span style={{ fontFamily: "'Fira Code', monospace", fontWeight: 700, fontSize: 13, color: '#1a1a2e' }}>{proj.title || `Project #${i + 1}`}</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div>
                    ...
                    {errors[i]?.description && <span style={{ fontSize: 12, color: '#ef4444', fontFamily: "'Fira Code', monospace", marginTop: 3, display: 'block' }}>{errors[i].description}</span>}
                  </div>
                  <div>
