...
export function AchievementsPage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f9fafb'}}>
      <Navbar active="Dashboard"/>
      <div style={{maxWidth:760,margin:'0 auto',padding:'32px 24px 80px'}}>
        <button onClick={()=>router.push('/home')} style={{display:'flex',alignItems:'center',gap:6,background:'none',border:'none',cursor:'pointer',color:'#6b7280',fontSize:13.5,marginBottom:24,padding:0,fontFamily:"'Fira Code', monospace"}}>← Back to Dashboard</button>
        <div style={{marginBottom:28}}>
          <h1 style={{fontFamily:"'Fira Code', monospace",fontWeight:800,fontSize:28,color:'#1a1a2e',marginBottom:4}}>🏅 {isEdit?'Edit Achievement':'Add Achievements'}</h1>
          <p style={{color:'#9ca3af',fontSize:14,fontFamily:"'Fira Code', monospace"}}>{isEdit?'Update achievement details':'Highlight your milestones and awards'}</p>
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      <Navbar active="Dashboard" />
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        {!isEdit && allAch.length>0 && (
          <div style={{marginBottom:28}}>
            <p style={{fontSize:12,fontWeight:700,letterSpacing:'0.5px',color:'#9ca3af',textTransform:'uppercase',marginBottom:12,fontFamily:"'Fira Code', monospace"}}>Saved Achievements ({allAch.length})</p>
            {allAch.map((a,i)=>(
              <div key={i} style={{background:'white',borderRadius:10,padding:'14px 18px',border:'1px solid #e5e7eb',marginBottom:8,display:'flex',alignItems:'center',gap:14}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <p style={{fontFamily:"'Fira Code', monospace",fontWeight:700,fontSize:14,color:'#1a1a2e',marginBottom:1}}>{a.title}</p>
                  {a.date&&<p style={{fontSize:12,color:'#9ca3af',fontFamily:"'Fira Code', monospace"}}>{a.date}</p>}
        {!isEdit && allAch.length > 0 && (
          <div style={{ marginBottom: 28 }}>
            ...
            {allAch.map((a, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 10, padding: '14px 18px', border: '1px solid #e5e7eb', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <button onClick={()=>router.push(`/profile/achievements?edit=${i}`)} style={{background:'none',border:'1.5px solid #e5e7eb',borderRadius:7,padding:'5px 12px',fontSize:12,color:'#6b7280',cursor:'pointer',fontFamily:"'Fira Code', monospace",fontWeight:600,flexShrink:0}}>✏️ Edit</button>
                <button onClick={()=>handleDelete(i)} disabled={loading} style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:7,padding:'5px 12px',fontSize:12,color:'#ef4444',cursor:'pointer',fontFamily:"'Fira Code', monospace",fontWeight:600,flexShrink:0}}>🗑 Delete</button>
        ...
        <form onSubmit={handleSave}>
          <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', padding: '28px 32px', boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
            ...
            {!isEdit&&(<button type="button" onClick={()=>{setEntries(p=>[...p,{...EMPTY_ACH}]);setErrors(p=>[...p,{}]);}} style={{width:'100%',padding:'12px',border:'2px dashed #e5e7eb',borderRadius:10,background:'transparent',cursor:'pointer',color:'#f97316',fontFamily:"'Fira Code', monospace",fontWeight:700,fontSize:13.5,display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginBottom:24}}>+ Add Another Achievement</button>)}
            <div style={{display:'flex',justifyContent:'flex-end',gap:12,marginTop:8}}>
              <button type="button" onClick={()=>router.push('/home')} style={{padding:'12px 24px',border:'1.5px solid #e5e7eb',borderRadius:10,background:'none',cursor:'pointer',color:'#6b7280',fontFamily:"'Fira Code', monospace",fontSize:14}}>Cancel</button>
              <button type="submit" disabled={loading} style={{padding:'12px 32px',background:'#1a1a2e',border:'none',borderRadius:10,color:'white',fontFamily:"'Fira Code', monospace",fontWeight:700,fontSize:14,cursor:loading?'not-allowed':'pointer',opacity:loading?0.7:1}}>{loading?'Saving...':isEdit?'✓ Update':'✓ Save'}</button>
  ...
  return(
    <div style={{minHeight:'100vh',background:'#f9fafb'}}>
      ...
      <div style={{maxWidth:760,margin:'0 auto',padding:'32px 24px 80px'}}>
        <button onClick={()=>router.push('/home')} style={{display:'flex',alignItems:'center',gap:6,background:'none',border:'none',cursor:'pointer',color:'#6b7280',fontSize:13.5,marginBottom:24,padding:0,fontFamily:"'Fira Code', monospace"}}>← Back to Dashboard</button>
        <div style={{marginBottom:28}}>
          <h1 style={{fontFamily:"'Fira Code', monospace",fontWeight:800,fontSize:28,color:'#1a1a2e',marginBottom:4}}>💼 {isEdit?'Edit Experience':'Add Experience'}</h1>
          <p style={{color:'#9ca3af',fontSize:14,fontFamily:"'Fira Code', monospace"}}>Add internships, jobs, and freelance work</p>
  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      <Navbar active="Dashboard" />
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        {!isEdit&&allExp.length>0&&(
          <div style={{marginBottom:28}}>
            <p style={{fontSize:12,fontWeight:700,letterSpacing:'0.5px',color:'#9ca3af',textTransform:'uppercase',marginBottom:12,fontFamily:"'Fira Code', monospace"}}>Saved ({allExp.length})</p>
            {allExp.map((e,i)=>(
              <div key={i} style={{background:'white',borderRadius:10,padding:'14px 18px',border:'1px solid #e5e7eb',marginBottom:8,display:'flex',alignItems:'center',gap:14}}>
                ...
                <div style={{flex:1}}><p style={{fontFamily:"'Fira Code', monospace",fontWeight:700,fontSize:14,color:'#1a1a2e',marginBottom:1}}>{e.role}</p><p style={{fontSize:13,color:'#9ca3af',fontFamily:"'Fira Code', monospace"}}>{e.company}</p></div>
                <button onClick={()=>router.push(`/profile/experience?edit=${i}`)} style={{background:'none',border:'1.5px solid #e5e7eb',borderRadius:7,padding:'5px 12px',fontSize:12,color:'#6b7280',cursor:'pointer',fontFamily:"'Fira Code', monospace",fontWeight:600,flexShrink:0}}>✏️ Edit</button>
                <button onClick={()=>handleDelete(i)} disabled={loading} style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:7,padding:'5px 12px',fontSize:12,color:'#ef4444',cursor:'pointer',fontFamily:"'Fira Code', monospace",fontWeight:600,flexShrink:0}}>🗑 Delete</button>
        {!isEdit && allExp.length > 0 && (
          <div style={{ marginBottom: 28 }}>
              ...
              <div key={i} style={{border:'1.5px solid #e5e7eb',borderRadius:12,padding:'20px 22px',marginBottom:16}}>
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
                    <div><label style={lbl}>Role *</label><input style={{...inp,border:`1.5px solid ${errors[i]?.role?'#ef4444':'#e5e7eb'}`}} placeholder="e.g. Software Engineer Intern" value={e.role} onChange={ev=>change(i,'role',ev.target.value)}/>{errors[i]?.role&&<span style={{fontSize:12,color:'#ef4444',fontFamily:"'Fira Code', monospace",marginTop:3,display:'block'}}>{errors[i].role}</span>}</div>
                    <div><label style={lbl}>Company *</label><input style={{...inp,border:`1.5px solid ${errors[i]?.company?'#ef4444':'#e5e7eb'}`}} placeholder="e.g. Google" value={e.company} onChange={ev=>change(i,'company',ev.target.value)}/>{errors[i]?.company&&<span style={{fontSize:12,color:'#ef4444',fontFamily:"'Fira Code', monospace",marginTop:3,display:'block'}}>{errors[i].company}</span>}</div>
          <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', padding: '28px 32px', boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
            {entries.map((e, i) => (
              <div key={i} style={{ border: '1.5px solid #e5e7eb', borderRadius: 12, padding: '20px 22px', marginBottom: 16 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {!isEdit&&(<button type="button" onClick={()=>{setEntries(p=>[...p,{...EMPTY_EXP}]);setErrors(p=>[...p,{}]);}} style={{width:'100%',padding:'12px',border:'2px dashed #e5e7eb',borderRadius:10,background:'transparent',cursor:'pointer',color:'#f97316',fontFamily:"'Fira Code', monospace",fontWeight:700,fontSize:13.5,display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginBottom:24}}>+ Add Another</button>)}
            <div style={{display:'flex',justifyContent:'flex-end',gap:12,marginTop:8}}>
              <button type="button" onClick={()=>router.push('/home')} style={{padding:'12px 24px',border:'1.5px solid #e5e7eb',borderRadius:10,background:'none',cursor:'pointer',color:'#6b7280',fontFamily:"'Fira Code', monospace",fontSize:14}}>Cancel</button>
              <button type="submit" disabled={loading} style={{padding:'12px 32px',background:'#1a1a2e',border:'none',borderRadius:10,color:'white',fontFamily:"'Fira Code', monospace",fontWeight:700,fontSize:14,cursor:loading?'not-allowed':'pointer',opacity:loading?0.7:1}}>{loading?'Saving...':isEdit?'✓ Update':'✓ Save'}</button>
