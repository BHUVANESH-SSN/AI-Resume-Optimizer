  ...
  return (
    <Modal title="Edit Profile Details" onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 12 }}>
        ...
        <button onClick={async () => { setSaving(true); await onSave({ full_name: f, email: e, phone: p, location: l, institute: i }); setSaving(false); }} disabled={saving} style={{ padding: '10px 28px', background: 'linear-gradient(135deg, #A78BFA 0%, #6c47ff 50%, #1a1a2e 100%)', border: 'none', borderRadius: 12, color: '#fff', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: `0 6px 20px ${C.accent}50` }}>
  ...
  return (
    <Modal title="Manage Skills" onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <button onClick={onClose} style={{ padding: '10px 24px', border: `1px solid ${C.border}`, borderRadius: 12, background: 'none', cursor: 'pointer', color: C.muted, fontFamily: "'Montserrat', sans-serif", fontSize: 14, fontWeight: 600 }}>Cancel</button>
        <button onClick={save} disabled={saving} style={{ padding: '10px 28px', background: 'linear-gradient(135deg, #A78BFA 0%, #6c47ff 50%, #1a1a2e 100%)', border: 'none', borderRadius: 12, color: '#fff', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: `0 6px 20px ${C.accent}50` }}>
  ...
  return (
    <div ref={reveal.ref} style={{ ...reveal.style, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 24 }}>
      ...
      <button onClick={onAdd}
        ...
        style={{ background: 'linear-gradient(135deg, #A78BFA 0%, #6c47ff 50%, #1a1a2e 100%)', color: '#fff', border: 'none', borderRadius: 14, padding: '12px 24px', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: 13.5, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, transition: 'all 0.2s cubic-bezier(.4,0,.2,1)', boxShadow: `0 6px 20px ${C.accent}40`, whiteSpace: 'nowrap' }}>
        {addLabel}
      </button>
    </div>
  );
    ...
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ minWidth: 420, maxWidth: 420, flexShrink: 0, borderRadius: 24, background: `linear-gradient(90deg, ${accentColor}, ${accentColor}80) top / 100% 4px no-repeat, ${C.surface}`, border: `1px solid ${hov ? accentColor + '55' : C.border}`, boxShadow: hov ? `0 25px 60px rgba(15,23,42,0.14), 0 0 0 1px ${accentColor}20` : '0 4px 18px rgba(15,23,42,0.07)', transform: hov ? 'translateY(-7px) scale(1.01)' : 'none', transition: 'all 0.3s cubic-bezier(.4,0,.2,1)', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '18px 22px 0', flexWrap: 'wrap' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, opacity: hov ? 1 : 0, transition: 'opacity 0.2s' }}>
          <button onClick={onEdit} style={{ background: 'linear-gradient(135deg, #A78BFA 0%, #6c47ff 50%, #1a1a2e 100%)', border: 'none', borderRadius: 9, padding: '6px 14px', fontSize: 12.5, color: '#fff', cursor: 'pointer', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, display: 'flex', alignItems: 'center', gap: 5 }}>
            <Pencil size={12} /> Edit
          </button>
  ...
  return (
    <div ref={reveal.ref} style={{ ...reveal.style, border: `2px dashed ${C.border}`, borderRadius: 20, padding: '48px 24px', textAlign: 'center', background: `linear-gradient(135deg, ${C.paper}, ${C.accentSoft}30)` }}>
      ...
      <button onClick={onAdd}
        ...
        style={{ background: 'linear-gradient(135deg, #A78BFA 0%, #6c47ff 50%, #1a1a2e 100%)', color: '#fff', border: 'none', borderRadius: 14, padding: '12px 28px', fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: `0 6px 20px ${C.accent}40`, transition: 'all 0.2s', display: 'inline-flex', alignItems: 'center', gap: 7 }}>
        <Plus size={16} /> {label}
      </button>
    </div>
  );
