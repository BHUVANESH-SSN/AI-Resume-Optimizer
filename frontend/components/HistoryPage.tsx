...
// ─── MAIN HISTORY PAGE ────────────────────────────────────────────────────────
export default function HistoryPage({ onGoBack }: { onGoBack: () => void }) {
  ...
  const scores = history.map((h) => h.match_score).filter((s): s is number => s != null);
        ...
        {!loading && !error && history.length > 0 && (
          <div className="stats-grid" style={{ marginTop: 0, marginBottom: 40 }}>
            ...
            <div className="stat-card fade-in-up fade-in-up-delay-2">
              ...
              <div className="stat-value" style={{ color: (bestScore ?? 0) >= 80 ? "#16a34a" : "var(--ink)" }}>
                {bestScore ?? "—"}
          ...
          {!loading && error && (
            <div className="error-banner" style={{
              ...
              <button className="btn-primary" onClick={fetchHistory}
                style={{
                  width: "auto", padding: "10px 24px", marginTop: 4,
                  display: "flex", alignItems: "center", gap: 6
                }}>
